...
public class RidesAndFaresExercise {
    ...
    public static class EnrichmentFunction
        ...
        private transient ValueState<RideAndFare> rideAndFareState;

        @Override
        public void open(Configuration config) throws Exception {
            ValueStateDescriptor<RideAndFare> descriptor = new ValueStateDescriptor<>("rideAndFare", RideAndFare.class);
            rideAndFareState = getRuntimeContext().getState(descriptor);
        }

        @Override
        public void flatMap1(TaxiRide ride, Collector<RideAndFare> out) throws Exception {
            RideAndFare rideAndFare = rideAndFareState.value();
            if (rideAndFare == null) {
                rideAndFare = new RideAndFare();
            }
            rideAndFare.ride = ride;
            rideAndFareState.update(rideAndFare);
            if (rideAndFare.ride != null && rideAndFare.fare != null) {
                out.collect(rideAndFare);
                rideAndFareState.clear();
            }
        }
